                ...
                <div class="card shadow">
                    <div class="card-header border-0">
                        <div class="row align-items-center">
                            <div class="col">
                                <h3 class="mb-0">Editar {{ $material->display_name }}</h3>
                                @if ($errors->any())
                    ...
                    <div class="card-body">
                        <form method="POST" enctype="multipart/form-data" action="{{ route('materials.update', $material->id) }}" class="pl-5 pr-5">
                            @csrf
                            {{ method_field('PUT') }}
                            <div class="form-group row">
                                <label for="Name" class="col-sm-3 col-form-label">Nombre del Material</label>
                                <div class="col-sm-9">
                                    <input type="text" class="form-control" name="name" value="{{ old('name', $material->display_name) }}" placeholder="Ej. Caucho" >
                                </div>
                            </div>
                            <div class="form-group row">
                            	<label for="Description" class="col-sm-3 col-form-label">Descripción</label>
                                <div class="col-sm-9">
                                    <textarea name="description" rows="5" cols="79" placeholder="Escribe la descripción del material">{{ old('description', $material->description) }}</textarea>
                                </div>
                            </div>
                            <div class="form-group row">
