        ...
        <div class="row mt-5">
            <div class="col">
                <div class="card shadow">
                    ...
                    <div class="card-body">
                        <form method="POST" enctype="multipart/form-data" action="{{ route('refereeTypes.update', $refereeType->id) }}" class="pl-5 pr-5">
                            @csrf
                            {{ method_field('PUT') }}
                            <div class="form-group row">
                                <label for="Name" class="col-sm-3 col-form-label">Nombre de la posición</label>
                                <div class="col-sm-9">
                                    <input type="text" class="form-control" name="name" value="{{ old('name', $refereeType->display_name) }}" placeholder="Ej. Juez de línea" >
                                </div>
                            </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <select class="form-control" name="sport_id" placeholder="Selecciona un deporte">
                                    ...
                                    @foreach ($sports as $s)
                                    <option {{ old('sport_id') == $s->id || $refereeType->sport_id == $s->id ? 'selected' : ''}} value="{{ $s->id }}">{{ $s->display_name }}</option>`
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-9">
                                    <textarea name="description" rows="5" cols="79" placeholder="Escribe la descripción del material">{{ old('description', $refereeType->description) }}</textarea>
                                </div>
                            </div>
                            <div class="form-group row">
